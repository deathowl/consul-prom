...
/usr/local/bin/consul kv get  prometheus-entrypoint.sh | bash -x
...
/usr/local/bin/consul-template \
       ...
       -template <(echo '{{key "etc/prometheus/sdiscovery/jmx_resourcemanager.yml"}}' ):/etc/prometheus/sdiscovery/jmx_resourcemanager.yml:"kill -HUP $(pidof prometheus)"
KOMMENT
